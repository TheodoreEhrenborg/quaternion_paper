...
The paper found an upper bound for the number
of such solutions: $57$. This paper seeks to
...
\todo[inline]{f,g should become a,b}
...
\end{center}
\caption{
Sample data for differentiating between equivalence classes
of identities. 
...
number of equivalence classes were used. 

To 
separate the identities into equivalence classes,
the program \emph{numeric\_comparison}
calculated the particular numeric values of 
identities. If two identities fulfilled
a different set of solutions, they were
different. 
For $n = 2$, a sample of this process
appears in Table~\ref{table_barcode}.
This program provided a lower
bound on the number of identities, but 
could not prove that two identities are 
equivalent.
...
This program provided an upper
bound on the number of identities,
but could not prove that two identities
are not equivalent.

The two programs agreed on the values for 
...
over~\cite{Ehrenborg_2018}'s estimate
...
Theorem~\ref{theorem_counting_4D} to
cases where $p$ and $m$ take on values
less than four.
\end{enumerate}
